...
public class HttpUtils {
    ...
    public static String execute(CloseableHttpClient httpClient, HttpUriRequest uriRequest, String charset) {
        ...
        try {
            ...
            resultStr = EntityUtils.toString(entity, CHARSET_UTF8);
            // 关闭流
            EntityUtils.consume(entity);
        } catch (IOException e) {
            logger.error("请求【" + uriRequest.getURI().toString() + "】出现异常：", e);
        } finally {
            if (response != null) {
                try {
                    response.close();
                } catch (IOException e) {
                    logger.error("关闭CloseableHttpResponse出现异常：", e);
                }
            }
        }
        return resultStr;
    ...
    private synchronized static void initWithoutSSL() {
        try {
            ...
            logger.error("初始化http连接池(不检测ssl)出现异常：", e);
        }
    }
}
